...
<% HttpSession sesion=request.getSession(false); if (sesion==null ||
    sesion.getAttribute("usuarioLogueado")==null) { response.sendRedirect(request.getContextPath()
    + "/index.jsp" ); return; } String pagina=request.getParameter("page"); if (pagina==null ||
    pagina.trim().isEmpty()) { pagina="inicio/inicio.jsp" ; } else { pagina=pagina.trim(); }
...
<body>
    ...
    <div class="sidebar p-3">
        ...
        <% Usuario usuarioLog=(Usuario) session.getAttribute("usuarioLogueado"); %>
            ...
            <div class="collapse <%= pagina.startsWith(" clientes") ? "show" : "" %>"
                id="submenuClientes">
            ...
            <div class="collapse <%= pagina.startsWith(" habitaciones") ? "show" : "" %>"
                id="submenuHabitaciones">

                <a href="${pageContext.request.contextPath}/LoginController?page=habitaciones/habitacionesRegistrar.jsp"
                    class="nav-link ms-4 <%= " habitaciones/habitacionesRegistrar.jsp".equals(pagina)
                    ? "active" : "" %>">
                ...
                <a href="${pageContext.request.contextPath}/LoginController?page=habitaciones/habitacionesConsultar.jsp"
                    class="nav-link ms-4 <%= " habitaciones/habitacionesConsultar.jsp".equals(pagina)
                    ? "active" : "" %>">
                    <i class="bi bi-search"></i> Consultar
                </a>
    ...
    <main class="fade-in">
        <% String titulo="HotelSys - " ; if (pagina.contains("inicio")) titulo +="Inicio" ; else if
            (pagina.contains("clientesRegistrar")) titulo +="Registrar Cliente" ; else if
            (pagina.contains("clientesConsultar")) titulo +="Consultar Clientes" ; else if
            (pagina.contains("habitacionesRegistrar")) titulo +="Registrar Habitación" ; else if
            (pagina.contains("habitacionesConsultar")) titulo +="Consultar Habitaciones" ; else if
            (pagina.contains("habitaciones")) titulo +="Habitaciones" ; else if
            (pagina.contains("reservas")) titulo +="Reservas" ; else if (pagina.contains("productos"))
            titulo +="Productos" ; else if (pagina.contains("productosCrear")) titulo
            +="Crear Productos" ; else if (pagina.contains("productosEditar")) titulo
            +="Editar Productos" ; else if (pagina.contains("configuracion")) titulo +="Configuración" ;
